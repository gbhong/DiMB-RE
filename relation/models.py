...
class BertForRelation(BertPreTrainedModel):
    def __init__(self, config, num_rel_labels, use_trigger=False):
    # def __init__(self, config, num_rel_labels, num_certainty_labels, multitask=True):
        ...
        #     self.layer_norm_certainty = BertLayerNorm(config.hidden_size)
        #     self.classifier_certainty = nn.Linear(config.hidden_size, self.num_certainty_labels)
        self.init_weights()

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, sub_idx=None, obj_idx=None, trg_idx=None, input_position=None):
        ...
        if trg_idx is not None:
            trg_output = torch.cat([a[i].unsqueeze(0) for a, i in zip(sequence_output, trg_idx)])
            rep = torch.cat((trg_output, sub_output, obj_output), dim=1)
        else:
        ...
        #     rep_certainty = self.layer_norm_certainty(first_token_tensor)
        #     rep_certainty = self.dropout(rep_certainty)
        #     logits_certainty = self.classifier_certainty(rep_certainty)

        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            # if self.multitask:
            #     loss_certainty = loss_fct(logits_certainty.view(-1, self.num_certainty_labels), labels_certainty.view(-1))
            #     loss += loss_certainty
            return loss
        else:
            # if self.multitask:
            #     return logits, logits_certainty
            return logits
