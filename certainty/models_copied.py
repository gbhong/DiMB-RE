...
class BertForRelation(BertPreTrainedModel):
    def __init__(self, config, num_certainty_labels, use_trigger=True):
        ...
        # TODO: switch hidden states for classification
        # 1. [CLS] 2. entity pair 3. [REL]
        if not use_trigger:
            self.layer_norm = BertLayerNorm(self.hidden_size * 2)
            self.classifier = nn.Linear(config.hidden_size * 2, self.num_labels)
        else:
            self.layer_norm = BertLayerNorm(self.hidden_size * 3)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, sub_idx=None, obj_idx=None, trg_idx=None, input_position=None):
        ...
        if not self.use_trigger:
            ...
            # rel_output = torch.cat([a[i].unsqueeze(0) for a, i in zip(sequence_output, trg_idx)])
            # cls_output = sequence_output[:, 0, :]
            rep = torch.cat((sub_output, obj_output), dim=1)
            # rep = sequence_output[:, 0, :]
            # rep = torch.cat([a[i].unsqueeze(0) for a, i in zip(sequence_output, trg_idx)])
        else:
            # rep = torch.cat([a[i].unsqueeze(0) for a, i in zip(sequence_output, trg_idx)])
            sub_output = torch.cat([a[i].unsqueeze(0) for a, i in zip(sequence_output, sub_idx)])
            obj_output = torch.cat([a[i].unsqueeze(0) for a, i in zip(sequence_output, obj_idx)])
            ...
            # cls_output = sequence_output[:, 0, :]
            rep = torch.cat((sub_output, obj_output, trg_output), dim=1)
